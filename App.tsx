...
import { Plus, Search, Gamepad2, Sparkles, Filter, PlayCircle, CheckCircle2, Lightbulb, Brain, Rocket, BookOpen, Code2 } from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    ...
    setIdeas(loadIdeas());
    setApps(loadApps());
    setLearnItems(loadLearning());
  }, []);

  useEffect(() => { if (games.length > 0) saveGames(games); }, [games]);
  useEffect(() => { if (ideas.length > 0) saveIdeas(ideas); }, [ideas]);
  useEffect(() => { if (apps.length > 0) saveApps(apps); }, [apps]);
  useEffect(() => { if (learnItems.length > 0) saveLearning(learnItems); }, [learnItems]);
